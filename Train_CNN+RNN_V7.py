...
X_images_after_L4 = [0]*20
for i in range(20):
    temp = tf.matmul(X_images_after_L3[i], W4) + b4
    X_images_after_L4[i] = temp
    


# < Fully Connected to 7 nodes(softmax) : Layer 5 >
# X_images_after_L5 : [?, 20, 7]
W5 = tf.get_variable("W5", shape=[3072, 7],
                     initializer=tf.contrib.layers.xavier_initializer())
b5 = tf.get_variable("b5", shape=[7],
                     initializer=tf.contrib.layers.xavier_initializer())
temp = X_images_after_L4[0]
temp = tf.matmul(temp, W5) + b5
X_images_after_L5 = temp
for i in range(1, 20, 1):
    temp = X_images_after_L4[0]
    temp = tf.matmul(temp, W5) + b5
    X_images_after_L5 = tf.concat([X_images_after_L5, temp], 1)



# < Summing up results of each frame : Layer 6 >
W6 = tf.get_variable("W6", shape=[20*7, 7],
                     initializer=tf.contrib.layers.xavier_initializer())
b6 = tf.get_variable("b6", shape=[7],
                     initializer=tf.contrib.layers.xavier_initializer())
logits = tf.matmul(X_images_after_L5, W6) + b6
...
outputs, _states = tf.nn.static_rnn(lstm, X_split, dtype=tf.float32)
    # Linear activation
    # Get the last output
logits, _state = tf.matmul(outputs[-1], W5) + b5, lstm.state_size # State size to initialize the stat
...
save_path = saver.save(sess, "./version_7_trial2.ckpt")
